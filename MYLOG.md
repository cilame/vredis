...
还是要设计一个主要用于分发任务的函数，以后考虑。
    ...
    # 可能与内存相关，考虑增加内存。或者直接从 redis 的启动配置文件修改成 “stop-writes-on-bgsave-error no”
    # 这样就可以忽略问题，让 redis 只是暂停部分时间后又重新开始，事实上并不影响任务的完整性。
    # 不过，最好的方式还是直接加内存。
...
#20190104
增加了配置默认参数的方法，让使用变得更加简单了。暂时还没有扩展到 pipe 脚本的功能也用配置的方式。
感觉如果把配置也藏起来，感觉 …… 貌似也不错。是个还不错的想法。晚上试着改改。后续真要这么改，真的就是一行代码的分布式了。
    # 参数放在环境变量的 HOME 路径下的 .vredis 文件内部。
    # 现在脚本里面的 pipe 实例也将会使用配置来处理了。这样就更加方便了。
    # 这个灵光一闪还真是不错，并且这样的话测试数据里面也没有必要加入敏感信息了。

#20190105
增加取数据的限制，这样可以快速检查部分数据情况。

#20190106
修复了命令行 vredis dump 下来的数据不能直接被 json 加载的问题。

#20190114
修复BUG：当执行的函数不返回时就不会对执行次数进行统计，并且会引发一个停止处理的问题。
另外，从便利性上看，发现如果发送任务的工作全部都交给 sender 端来执行时就会不太方便。
因为哪怕是多线程发送任务，也会占用一定的时间，如果说任务都由着 worker 端生成的话可能会更好一些。
不过也要考虑 worker 的任务生成可能只能是单线程的情况，任务生成的效率上会有影响。
    # 暂时还没有进行对 worker 进行任务生成的功能开发。
    # 目前所有的 task 全部都需要通过 sender 端进行发送。

#20190319
废弃LOG_ITEM，后期想了想之前的这个设计太傻逼了一点，不如将这个接口直接换成一个更加方便的功能
改成了DUMPING，意味为在实时输出的情况下，可以将收集的文件直接在本地生成一个json文件。
并且，在命令行工具中增加了一些对 stat 命令更加友好的操作，让获取数据变得更加方便一些。
    # 目前处理的内容意在对使用者更加友好的基础上展开。

#20190320
修复 KEEPALIVE 任务发送时候存在的巨大问题。现在已经是正常的等待任务发送完毕再进行任务的发送了。
新的问题就是怎么在该模式下事先检查是否存在存活的 worker。否则发送几百万的数据结果最后没有任务队列接收就尴尬了。

#20190331
发现一个严重的问题，check_connect_worker 并不一定能检测到是否连接
...
非常遗憾，在最开始测试这个函数的时候可能太过顺利，甚至没有出现错误，导致我认为这个函数可以使用。
不过，现在就只是要保证 worker 端尽量稳定开启状态了。等以后redis解决问题后再考虑修改检查方法。
    # 新增了一个功能，任务函数中出现了的其他任务函数能够正常执行
    # 且那些任务函数内的任务函数变成发送任务的函数，传入总任务管道里面等待分配执行。
    # 新功能将更大的拓展了解析再执行的功能，该工具也将变成一个很好使用的工具。
```